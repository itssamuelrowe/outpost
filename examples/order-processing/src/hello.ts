 ...
 *   2. Running the same workflow identifier again does not repeat completed
 *      steps — their results are memoised and reused.
 *   3. Every material transition is recorded, which you can see in the dumped
 *      state and audit events at the end.
 *
 ...
 *
 * The class is marked with `@Workflow`, and each durable step is a method marked
 * with `@Step`. The `run` method is the lifecycle method: it calls the steps in
 * order and returns the workflow's final result.
 */
...
class GreetUser {
   ...
   *
   * Because it is a step, its return value is persisted. If the workflow is run
   * again with the same identifier, this method will not execute a second time;
   * the stored greeting is returned instead.
   */
  @Step()
  ...
  /**
   * The second durable step. In a real application this might send an email or a
   * message; here it simply prints the greeting to show the step ran.
   */
  @Step()
...
const main = async (): Promise<void> => {
  ...
  for (const schedule of snapshot.schedules) {
    console.log(`  - #${schedule.scheduleIdentifier} [${schedule.status}] for ${schedule.stepKey ?? "workflow"}`);
  }
