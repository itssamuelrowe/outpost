 ...
 * production, this file actually runs. It uses in-process fakes for the
 * external services, so you can watch a durable workflow execute — including
 * the ambiguous-state probe recovery — without standing up AWS or any real
 * third parties.
 *
 ...
 *
 * Or with no setup at all: yarn demo
 */
 ...
 * falls back to the in-memory adapter, which needs no setup but keeps state
...
async function createStorage(): Promise<{ storage: StorageAdapter; pool: Pool | null }> {
    ...
    if (!databaseUrl) {
        console.log(
            "No OUTPOST_DEMO_DB_URL set; using in-memory storage (state is not persisted).",
        );
