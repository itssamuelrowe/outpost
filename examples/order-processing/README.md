...
Start with **`src/demo.ts`**, then explore the focused examples below.
...
Each of these runs on in-memory (or file) storage with nothing to configure, via
`yarn <script>`. They use the **class-and-decorator** style, which is the default
and what we recommend.

- **`hello`** — the smallest workflow, run twice to show memoisation.
- **`durable-sleep`** — a durable pause that survives a process restart.
- **`child-workflows`** — a parent that fans out to one durable child per item.
- **`deterministic-values`** — `context.now` and `context.randomUUID`, stable across resumes.
- **`workflow-management`** — inspect, list, and cancel executions.
- **`serialization-recipes`** — persist rich values (`Date`, `Map`, a custom `Money`).
- **`cron-backfill-update`** — backfill a past window and update a schedule in place.

### Functional-style examples

The two demos below use the **functional** authoring style (`engine.defineWorkflow`
with `context.step`) instead of decorators. The behaviour is identical; these
exist to show the alternative for people who prefer plain functions.

- **`functional-style`** — the checkout workflow written as a plain function.
- **`functional-cron`** — durable cron wired up functionally (`defineWorkflow` +
  `registerCron` + `onCronFire`).
