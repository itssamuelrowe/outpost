...
export interface SqsWorkflowConsumerOptions {
   ...
   * failures. The message is not deleted on failure, allowing SQS to redeliver
   * it according to the queue's redrive policy.
   */
  onHandlerError?: (message: WorkflowMessage, error: unknown) => void;
}

/**
 * Consumes workflow messages from an SQS queue and dispatches them to a handler.
 *
 * The consumer long-polls to reduce empty receives, deletes a message only
 * after its handler succeeds, and leaves redelivery and dead-letter behaviour to
 * the queue's own configuration. It deliberately holds no engine internals; the
 * handler is where the application wires the message to a workflow run.
 */
export class SqsWorkflowConsumer {
   ...
   * delete those whose handler succeeded. Exposed separately so tests can drive
   * one cycle deterministically without an unbounded loop.
   */
