...
export interface ChaosRule {
    ...
    attempts?: number[];
    /**
     * When the fault is injected relative to the step function. Defaults to
     * before.
     */
    timing?: ChaosTiming;
    /**
     * The error thrown when the rule matches. When omitted and a delay is set,
     * the rule only delays and does not throw.
     */
    error?: () => Error;
    /**
     * A delay, in milliseconds, applied before the fault. Use this to hold a
     * step long enough that its lease expires and another worker can take
     * over.
     */
