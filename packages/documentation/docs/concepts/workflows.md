...
The input you pass and the output `run` returns are both **persisted**, so by default they must be plain JSON data: numbers, strings, booleans, `null`, arrays, and plain objects made of those. A raw `Date`, `Map`, or class instance is rejected by default, because it cannot be faithfully saved as-is. (Steps follow the same rule for their results; see [Steps](./steps.md#what-a-step-can-return).)

If you want to carry richer types such as `Date`, `Map`, `Set`, or `bigint`, register them on a [serialization registry](./serialization.md) and the engine persists and restores them everywhere. The strict default stands until you opt in.
...
If you would rather not use classes, Outpost has an equivalent [functional style](../function-style/functional-workflows.md). It behaves identically; choose whichever reads better for you.
