...
3. **Probe if the last attempt was ambiguous.** If a previous attempt ended in the `AMBIGUOUS` state and the step has a probe, the probe runs _before_ your function:
...
- **Determinism** is about the code _between_ steps, which re-runs on every resume. See [Writing deterministic steps](./writing-deterministic-steps.md).
