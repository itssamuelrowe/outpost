...
The dangerous window is **between the API call returning and the row being written.** Trace the restart again: we call Shopify, Shopify creates the order and returns, and then the container restarts _before_ the `externalCalls` row is committed. On redelivery we check the table, find nothing, and call Shopify a second time. Duplicate order, despite all our care. The idempotency key saves us **only if Shopify honours one for order creation**, which many storefront setups do not.
...
Even once your individual writes are safe, a durable engine still saves you from hand-building durable waits, retry scheduling, an audit trail, chaos-tested recovery, and optional-step handling. See [What you get beyond safe writes](./beyond-safe-writes.md).
