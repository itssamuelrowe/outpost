 ...
 *
 * Correctness rests on two mechanisms. First, step claiming uses
 * `SELECT ... FOR UPDATE SKIP LOCKED` inside a transaction, so concurrent
 * workers cannot both claim the same runnable step. Second, commits and
 * failures are guarded by a fence token, so a worker whose lease has expired
 * and been taken over cannot overwrite the newer owner's result.
 *
...
export class MysqlStorage implements StorageAdapter {
  ...
  /**
   * Creates the schema by executing the versioned `.sql` files shipped with the
   * package. Each statement uses `CREATE TABLE IF NOT EXISTS`, so calling this
   * repeatedly is safe.
   */
  ...
  /** Runs the supplied function inside a transaction, committing or rolling back. */
    ...
    return this.withinTransaction(async (connection) => {
      ...
      await connection.query(
        `INSERT INTO \`outpostSteps\`
         ...
         VALUES (?, ?, ?, 0, ?, 0, ?, ?)
         ON DUPLICATE KEY UPDATE \`workflowIdentifier\` = \`workflowIdentifier\``,
        [workflowIdentifier, stepKey, StepStatus.PENDING, maxAttempts, timestamp, timestamp],
      );
      ...
      await connection.query(
        ...
        [StepStatus.RUNNING, attempt, fenceToken, newLease, timestamp, workflowIdentifier, stepKey],
      );
  ...
  public async releaseStep(
    ...
    const [result] = await this.pool.query(
      ...
      [StepStatus.PENDING, timestamp, workflowIdentifier, stepKey, fenceToken, StepStatus.RUNNING],
    );
    return (result as { affectedRows: number }).affectedRows > 0;
  }
    ...
    return this.withinTransaction(async (connection) => {
      ...
      await connection.query(
        `INSERT INTO \`outpostSchedules\`
         ...
         VALUES (?, ?, ?, ?, NULL, ?)`,
        [workflowIdentifier, timerKey, formatUtcDateTime(runAt), ScheduleStatus.PENDING, timestamp],
      );
      return { runAt };
    });
  }
  ...
  public async scheduleTimer(
    ...
    await this.pool.query(
      `INSERT INTO \`outpostSchedules\`
         (\`workflowIdentifier\`, \`stepKey\`, \`runAt\`, \`status\`, \`payload\`, \`createdAt\`)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [workflowIdentifier, stepKey, formatUtcDateTime(runAt), ScheduleStatus.PENDING, payload, timestamp],
    );
  }
