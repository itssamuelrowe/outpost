...
import { ClassifyError, Probe, Step, Workflow } from "../src/decorators/workflow.decorator.js";
...
describe("decorator-based authoring", () => {
    ...
    it("runs a workflow passed as an instance with injected dependencies", async () => {
        ...
        expect(storage.getStep("workflow-1", "compose-greeting")?.status).toBe(
    ...
    it("resolves an ambiguous step through a probe declared in the decorator", async () => {
        ...
        class CreateOrder {
            @Step({
                ...
                probe: async () =>
                    state.sideEffectOccurred ? { orderIdentifier: "SHOP-1" } : null,
            })
