...
export interface Step {
    ...
    maxAttempts: number;
    /**
     * The serialized memoized result of a successful or fallback outcome,
     * stored as binary.
     */
    output: Buffer | null;
    /**
     ...
     * present the token from their own claim; a mismatch indicates a stale
     * worker and the write is rejected.
     */
    fenceToken: number;
    /**
     * The instant until which the current lease is valid, or `null` when
     * unleased.
     */
    lockedUntil: Date | null;
    /**
     * The instant the step's result was committed, or `null` if not yet
     * completed.
     */
