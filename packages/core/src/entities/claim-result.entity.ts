 ...
 *  - The step is already terminal, in which case `cachedResult` holds the
 *    memoized output and `claimed` is `false`.
...
export interface ClaimResult {
   ...
   * optional failure). The output is stored as a binary buffer. `completedAt` is
   * the instant the result was committed, used to evaluate an optional result
   * time-to-live. Present only when the step should not be executed again.
   */
