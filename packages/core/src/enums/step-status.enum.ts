 ...
 *
 * The {@link StepStatus.AMBIGUOUS} state is central to the library's purpose: it
 * records that a step attempt failed in a way that leaves the downstream side
 * effect unknown (for example, a network timeout or a 5xx response). When a step
 * is recovered from this state, the engine consults a probe function to decide
 * whether the effect already happened before deciding to execute again.
 */
export enum StepStatus {
  ...
  COMPLETED = "COMPLETED",
  /** The step failed permanently and the failure is propagated to the workflow. */
  FAILED = "FAILED",
  /** An optional step failed permanently; the workflow continues with a fallback. */
