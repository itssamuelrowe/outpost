 ...
 *
 * The engine deliberately does not silently re-run the step, because a step
 * that ran earlier may have committed downstream steps that consumed its old
 * value, and re-running could duplicate real side effects. Instead this error
 * is raised so the workflow author can decide how to recover, for example by
 ...
 *
 * The engine catches this at the top of the run, marks the workflow as
 * suspended rather than failed, and returns. The scheduler later resumes the
 * workflow once the sleep timer becomes due. Application code should not catch
 * this error.
 */
...
/**
 * Raised when {@link WorkflowEngine.run} is called for a workflow that has been
 * cancelled. A cancelled workflow is terminal and must not be resumed, so the
 * engine refuses rather than silently running it. Callers that may race with a
