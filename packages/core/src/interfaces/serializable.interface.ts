 ...
 * guarantee: code can still bypass it with `as` casts. For a runtime safety net,
 * enable {@link WorkflowEngineOptions.validateSerializable}.
 */
 ...
 * such as a step's return object, because the compiler can check every property.
 * However, it rejects ordinary `interface` declarations: TypeScript does not
 * give interfaces an implicit index signature, so `interface OrderInput { ... }`
 * does not match `{ [key: string]: Serializable }` even when every field is in
 * fact serializable.
 *
 * To avoid forcing every consumer to rewrite their input interfaces as `type`
 * aliases, workflow input and output are constrained to this friendlier type. It
 * still rejects the values that are obviously wrong to persist at the top level
 * (functions, `undefined`, `bigint`, `symbol`), while accepting any object or
 * array shape. Deep serializability of these values is verified at run time by
 * the engine's optional `validateSerializable` check.
 */
