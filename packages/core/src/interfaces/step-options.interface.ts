...
export interface StepOptions<TResult extends Serializable = Serializable> {
     ...
     * the engine probes on recovery. Defaults to treating every error as
     * definite.
     */
    classifyError?: (error: unknown) => FailureKind;
    /**
     * Middleware applied to this step in addition to any engine-wide
     * middleware.
     */
     ...
     *
     * By default a completed step's result never expires: it is returned on
     * every resume. When this is set, a resume that reaches the step after its
     * result is older than the TTL does not return the stale value and does not
     * silently re-run the step. Instead the engine raises a
     * `StepResultExpiredError`, so the workflow can decide what to do
     * (regenerate, compensate, or fail).
     *
     * Use this for steps that produce a perishable resource, such as a signed
     * URL or a short-lived token. See the "Time-sensitive resources"
     * documentation.
     */
    resultTtlMilliseconds?: number;
    /**
     * An optional predicate that decides whether a memoized result is still
     * valid.
     *
     * It is called with the decoded prior result when a resume reaches an
     * already-completed step. Returning `true` keeps the result; returning
     * `false` causes the engine to raise a `StepResultExpiredError` rather than
     * return the stale value or re-run the step. Like `resultTtlMilliseconds`,
     * this only ever applies to steps that opt in.
     */
