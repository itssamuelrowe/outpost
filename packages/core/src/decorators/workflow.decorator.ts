 ...
 * method `createOrder`) is discovered automatically even without this decorator.
 * You may still add `@Probe()` to such a method as an explicit marker that it is
 * part of the workflow; with no argument, the target step is inferred from the
 * method name. Pass a step name to associate a differently named method.
 *
 * @param stepMethodName The step method this probes. When omitted, it is
 *   inferred from this method's name, which must follow the `probe<Step>` form.
 *
 * @example
 * ```ts
...
/**
 * Derives the conventional probe method name for a step method: `probe` followed
 * by the capitalized step method name (for example, `createOrder` →
 * `probeCreateOrder`).
 */
