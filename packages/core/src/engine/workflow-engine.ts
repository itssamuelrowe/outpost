...
export interface WorkflowEngineOptions {
     ...
     * otherwise be silently swallowed to preserve the original error. Defaults
     * to a no-op.
     */
...
export class WorkflowEngine {
     ...
     */
    private readonly inFlightLeases = new Map<
        string,
        { workflowIdentifier: string; stepKey: string; fenceToken: number }
    >();
    ...
    private resolveWorkflow<TInput extends SerializableInput, TOutput extends SerializableInput>(
        ...
        // functional definition; anything else is a decorator-based class/instance.
        ...
        return buildWorkflowFromClass<TInput, TOutput>(nameOrClassOrInstance);
    }
    ...
    >(
        ...
        // A cancelled workflow is terminal and must not be resumed; refuse rather
        // than silently running it. A completed workflow, by contrast, is safe to
        // re-run: its steps are memoised, so re-running is cheap and is exactly what
        // enables opt-in result revalidation on resume.
        const existing = await this.storage.getWorkflow(workflowIdentifier);
        if (existing && existing.status === WorkflowStatus.CANCELLED) {
            throw new WorkflowCancelledError(workflowIdentifier);
        }
    ...
    private createWorkflowContext(workflowIdentifier: string): WorkflowContext {
        return {
            ...
            runChild: async <TInput extends SerializableInput, TOutput extends SerializableInput>(
                childKey: string,
                nameOrClass: string | Constructor | object,
                input: TInput,
                options?: ChildWorkflowOptions,
    ...
    >(
        ...
        const childIdentifier =
            options?.workflowIdentifier ??
            composeChildWorkflowIdentifier(parentWorkflowIdentifier, childKey);
        ...
        await this.storage.logEvent(
            ...
            {
                childKey,
                childWorkflowIdentifier: childIdentifier,
                workflowName: name,
            },
        );
        ...
        const engine = this;
        return {
            workflowIdentifier: childIdentifier,
            async result(): Promise<TOutput> {
                ...
                await engine.storage.logEvent(
                    ...
                    { childKey, childWorkflowIdentifier: childIdentifier },
                );
                return output;
            },
        };
     ...
     */
    public async getWorkflowStatus(workflowIdentifier: string): Promise<WorkflowStatus | null> {
        const workflow = await this.storage.getWorkflow(workflowIdentifier);
    ...
    public async describeWorkflow<
        TInput extends SerializableInput = SerializableInput,
        TOutput extends SerializableInput = SerializableInput,
    >(workflowIdentifier: string): Promise<WorkflowDescription<TInput, TOutput> | null> {
        const workflow = await this.storage.getWorkflow(workflowIdentifier);
    ...
    public async getWorkflowResult<TOutput extends SerializableInput = SerializableInput>(
        workflowIdentifier: string,
        options: { throwIfNotComplete?: boolean } = {},
    ): Promise<TOutput | null> {
        const workflow = await this.storage.getWorkflow(workflowIdentifier);
        if (!workflow) {
            if (options.throwIfNotComplete) {
                throw new Error(`Workflow "${workflowIdentifier}" does not exist.`);
            }
            return null;
        }
        ...
        if (options.throwIfNotComplete) {
            if (workflow.status === WorkflowStatus.FAILED) {
                throw new Error(
                    `Workflow "${workflowIdentifier}" failed: ${workflow.error ?? "unknown error"}`,
                );
            }
            throw new Error(
                `Workflow "${workflowIdentifier}" has not completed (status: ${workflow.status}).`,
            );
        }
        return null;
    }
     ...
     * queries (via {@link WorkflowFilter.parentWorkflowIdentifier}).
     */
     ...
     * not resumed past it.
     */
    public async cancelWorkflow(workflowIdentifier: string): Promise<boolean> {
        const workflow = await this.storage.getWorkflow(workflowIdentifier);
        if (!workflow) {
            return false;
        }
        ...
        await this.storage.setWorkflowStatus(workflowIdentifier, WorkflowStatus.CANCELLED);
        await this.storage.logEvent(workflowIdentifier, null, EventType.WORKFLOW_CANCELLED, {});
        return true;
    }
    ...
    private describe<TInput extends SerializableInput, TOutput extends SerializableInput>(
        ...
        return {
            ...
            parentWorkflowIdentifier: workflow.parentWorkflowIdentifier,
     ...
     * no instance state, and is referenced from the failure-handling paths
     * above.
     */
 ...
 *
 * This is the whole of the "decorators are a thin wrapper" promise: the
 * metadata recorded by {@link Workflow} and {@link Step} is read here, and each
 * decorated step method is wrapped so that invoking it from the lifecycle
 * method routes transparently through `context.step` with the configured
 * options. Nothing in the core execution engine needs to know decorators
 * exist.
 *
 ...
 *   injection).
 */
function buildWorkflowFromClass<
    TInput extends SerializableInput,
    TOutput extends SerializableInput,
>(
...
/**
 * Returns the method name if the instance has a callable method with it, else
 * undefined.
 */
 ...
 * identifier and the stable child key. Because the derivation is pure, a resume
 * of the parent addresses exactly the same child execution, which the engine
 * then memoises rather than starting afresh.
 */
function composeChildWorkflowIdentifier(
    parentWorkflowIdentifier: string,
    childKey: string,
): string {
    return `${parentWorkflowIdentifier}::child::${childKey}`;
}

/**
 * The default UUID generator: the platform `crypto.randomUUID` when available,
 * with a sufficiently-unique fallback for older runtimes. Used by
 * {@link WorkflowContext.randomUUID}.
 */
function defaultUuid(): string {
    ...
    // Fallback: RFC-4122-shaped v4 string built from Math.random. Adequate when a
    // platform lacks crypto; supply your own `uuid` option for stronger guarantees.
