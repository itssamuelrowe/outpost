 ...
 * run time. The message names the offending path so the developer can locate it.
 */
 ...
 *
 * This is the runtime counterpart to the compile-time `Serializable` type. It is
 * used only when the engine is configured with `validateSerializable: true`, as
 * a safety net for values that slipped past the type system (for example through
 * an `as` cast). It walks the value and rejects anything that JSON persistence
 * cannot faithfully round-trip: functions, symbols, `undefined`, `bigint`, and
 * common non-plain objects such as `Date`, `Map`, `Set`, and class instances.
 *
...
function walk(value: unknown, path: string, description: string, seen: Set<object>): void {
  ...
  if (prototype !== Object.prototype && prototype !== null) {
    ...
    throw new NonSerializableValueError(description, path, `is an instance of ${constructorName}`);
  }
