...
engine.defineWorkflow("create-order-and-notify", async (context, input) => {
  ...
  await context.step(
    "notify-email",
    async () => email.send(input.customerEmail, "Your order is confirmed", `Order ${input.orderId}`),
    { maxAttempts: 5 },
  );
...
await context.step(
  ...
  async () => whatsApp.sendMessage(input.customerPhone, `Order ${input.orderId} confirmed!`),
  {
    maxAttempts: 5,
    classifyError,
    probe: async () => ((await whatsApp.wasDelivered(messageReference)) ? { sent: true } : null),
  },
);
```
